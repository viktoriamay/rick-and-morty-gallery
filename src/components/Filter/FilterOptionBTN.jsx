...
export const FilterOptionBTN = ({ optionName, task, updatePageNumber, index, name,updateStatus, status, option,checked, updateSpecies, updateGender }) => {
...
<div className="option_btn__wrapper">
  <input type="radio" checked={checked}  name={name} id={`${name}-${index}`} className="option_btn__radio"  />
  <label onClick={() => {
      task(optionName, updateSpecies, updateGender)
    }} htmlFor={`${name}-${index}`} className="option_btn__label">{optionName}</label>
</div>
  );
};